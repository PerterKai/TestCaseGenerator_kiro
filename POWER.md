...
- 图片分析支持两种方式：外部多模态LLM API（推荐，支持多线程并发）或 agent 内置视觉能力
...
> 请选择用例生成模式：
> 1. **文档+图片模式（外部LLM）**（推荐）— 解析文档文字和图片，图片通过外部多模态LLM API分析（如GPT-4o、Claude等），支持多线程并发，速度快且不消耗Agent tokens
> 2. **文档+图片模式（内置）**（高 tokens 消耗）— 解析文档文字和图片，图片通过 Agent 内置视觉能力逐一分析
> 3. **文档模式**（低 tokens 消耗）— 仅解析文档文字内容，跳过图片分析

- 用户选择"文档+图片模式（外部LLM）" → 执行步骤 2，然后调用 `configure_llm_api` 打开GUI配置外部API，配置完成后调用 `process_images_with_llm` 批量处理
- 用户选择"文档+图片模式（内置）" → 正常执行步骤 2 和步骤 3
...
### 步骤 3: 图片处理（支持跨 session）

#### 方式A: 外部LLM API（推荐，"文档+图片模式（外部LLM）"）
