...
### 自动 Review 最低轮次
**固定3轮分维度 Review，不允许跳过。第3轮仍有修改则追加第4轮。**
- 第1轮：覆盖完整性 + 图片覆盖
- 第2轮：边界充分性 + 异常覆盖
- 第3轮：步骤可执行性 + 预期结果明确性
...
#### 第1轮 Review：功能覆盖（覆盖完整性 + 图片覆盖）
1. 调用 `get_module()` （不传参数）获取模块列表概览（名称+用例数）
2. 逐模块调用 `get_module(module_name="模块名")` 获取单个模块的完整用例，专注检查：
   - 覆盖完整性: 文档中提到的每个功能点、接口、字段是否都有对应用例？
   - 图片覆盖: 流程图中的每条路径、状态图中的每个转换是否都有用例？
3. 发现问题后，逐模块调用 `save_testcases(append_module=修改后的单个模块对象)` 保存
4. 如发现重复或空模块，调用 `delete_module(module_names=["模块名"])` 删除
5. **本轮结束时，输出修改统计**：修改了 X 个模块，新增 X 个用例，修改 X 个用例，删除 X 个用例

#### 第2轮 Review：反向场景（边界充分性 + 异常覆盖）
1. 调用 `get_module()` 获取模块列表概览
2. 逐模块调用 `get_module(module_name="模块名")` 获取单个模块，专注检查：
   - 边界充分性: 每个输入字段是否考虑了边界值（最大值、最小值、空值、特殊字符）？
   - 异常覆盖: 每个操作是否考虑了失败场景（网络异常、超时、权限不足、数据不存在）？
3. 发现问题后，逐模块调用 `save_testcases(append_module=修改后的单个模块对象)` 保存
4. **本轮结束时，输出修改统计**：修改了 X 个模块，新增 X 个用例，修改 X 个用例，删除 X 个用例

#### 第3轮 Review：质量打磨（步骤可执行性 + 预期结果明确性）
1. 调用 `get_module()` 获取模块列表概览
2. 逐模块调用 `get_module(module_name="模块名")` 获取单个模块，专注检查：
   - 步骤可执行性: 每个步骤是否清晰、可操作？接口调用是否明确了请求方式、路径和关键参数？
   - 预期结果明确性: 预期结果是否具体、可验证？是否包含具体返回值或错误提示文案（不能用"操作失败"等模糊描述）？
3. 发现问题后，逐模块调用 `save_testcases(append_module=修改后的单个模块对象)` 保存
4. **本轮结束时，输出修改统计**：修改了 X 个模块，新增 X 个用例，修改 X 个用例，删除 X 个用例

#### 量化退出检查
- 如果第3轮修改数 > 0（即仍有用例被修改或新增），**必须追加第4轮全维度复查**：
  1. 对第3轮中修改过的模块，逐个调用 `get_module(module_name="模块名")` 获取并重新检查全部6个维度
  2. 逐模块保存修改
  3. 输出修改统计
  4. 如果第4轮修改数仍 > 0，可结束（最多4轮），但需在概述中标注"第4轮仍有修改，建议用户重点关注"
- 如果第3轮修改数 = 0，直接进入阶段5
