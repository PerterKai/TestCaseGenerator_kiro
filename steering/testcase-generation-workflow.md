...
逐一处理每张图片（每次只处理一张）：
   a. 调用 `get_pending_image` 获取下一张待处理图片
      - **v7.0**: 返回图片的位置上下文（所属章节、上下文类型如 paragraph/table_cell）
   ...
   c. 调用 `submit_image_result(image_id, analysis)` 提交结果
      - **v7.0**: 分析结果会带有章节标记写入 markdown，格式为：
        ```
        <!-- 图片分析开始: {image_id} | 章节: {section} -->
        {analysis}
        <!-- 图片分析结束: {image_id} -->
        ```
   d. 重复直到所有图片处理完毕
   e. **v7.0 新增**: 调用 `verify_image_positions` 验证所有图片分析结果位置正确
      - 检查每张图片的分析结果是否在正确的章节位置
      - 返回验证报告，包含错误和警告信息
