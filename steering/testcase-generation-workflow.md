...
| 模式 | 说明 | tokens 消耗 |
|------|------|-------------|
| 文档+图片模式（外部LLM） | 解析文字+图片，图片通过外部多模态LLM API分析，支持多线程 | 低（使用外部API） |
| 文档+图片模式（内置） | 解析文字+图片，图片通过Agent内置视觉逐一分析 | 高 |
| 文档模式 | 仅解析文字内容，跳过图片分析 | 低 |

- 用户选择"文档+图片模式（外部LLM）" → 执行阶段2，然后调用 `configure_llm_api` 打开GUI配置外部API，配置完成后调用 `process_images_with_llm` 批量处理图片
- 用户选择"文档+图片模式（内置）" → 执行阶段2 + 阶段2.5（逐一图片处理）
...
#### 方式A: 外部LLM API（推荐，仅"文档+图片模式（外部LLM）"）
